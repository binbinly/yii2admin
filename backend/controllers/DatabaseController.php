...
class DatabaseController extends BaseController{
    ...
    public function actionIndex($type = 'export'){
        ...
        switch ($type) {
            ...
            case 'import':
                //列出备份文件列表
                $path = realpath(Yii::$app->params['web']['DATA_BACKUP_PATH']);//var_dump($path);exit;
    ...
    public function actionDel($time = 0){
        if($time){
            ...
            $path  = realpath(Yii::$app->params['web']['DATA_BACKUP_PATH']) . DIRECTORY_SEPARATOR . $name;
    ...
    public function actionExport(){
        ...
        if(Yii::$app->request->isPost && !empty($tables) && is_array($tables)){ //初始化
            //读取备份配置
            $config = array(
                'path'     => realpath(Yii::$app->params['web']['DATA_BACKUP_PATH']) . DIRECTORY_SEPARATOR,
                'part'     => 20971520,
                'compress' => 1,
                'level'    => 9,
            );
